...
func (this *GenerateCy) genPlainTmplInstClses() {
}

func (this *GenerateCy) genTydefTmplInstClses() {
...
func (this *GenerateCy) genTemplateInstant(tmplClsCursor, argClsCursor clang.Cursor) {
	// tmplArgClsName := argClsCursor.Spelling()
	// tmplClsName := tmplClsCursor.Spelling()

	this.cp.APf("body", "struct %s  {", argClsCursor.Spelling())
...
func (this *GenerateCy) genTemplateMethod(cursor, parent clang.Cursor, argClsCursor clang.Cursor) {
	...
	cp.APf("body", "type TC_%s_%s_%d = func(voidptr) %s",
		clsName, validMethodName, midx, retstr)
	cp.APf("body", "func (this *%s) %s%d() *%s {",
	...
	cp.APf("body", "    fnobj := TC_%s_%s_%d(0)", clsName, validMethodName, midx)
	cp.APf("body", "    fnobj = qtrt.symQtfunc6(\"C_%s_%s_%d\")",
...
func (this *GenerateCy) genTemplateInterface(tmplClsCursor, argClsCursor clang.Cursor) {
...
func (this *GenerateCy) genTemplateInterfaceSignature(cursor, parent clang.Cursor, argClsCursor clang.Cursor) {
	clsName := argClsCursor.Spelling()
	elemClsName := clsName[:strings.LastIndexAny(clsName, "LHSM")]
