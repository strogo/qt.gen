...
func NewGenerateCy(qtdir, qtver string) *GenerateCy {
	this := &GenerateCy{}
...
func (this *GenerateCy) initBlocks() {
...
func (this *GenerateCy) genClass(cursor, parent clang.Cursor) {
	if false {
...
func (this *GenerateCy) final(cursor, parent clang.Cursor) {
	// log.Println(this.cp.ExportAll())
	this.saveCode(cursor, parent)

	this.initBlocks()
}
func (this *GenerateCy) saveCode(cursor, parent clang.Cursor) {
	// qtx{yyy}, only yyy
	file, line, col, _ := cursor.Location().FileLocation()
	if false {
		log.Printf("%s:%d:%d @%s\n", file.Name(), line, col, file.Time().String())
...
func (this *GenerateCy) saveCodeToFile(modname, file string) {
	// qtx{yyy}, only yyy
	...
	// gofmt the code
	cmd := exec.Command("go", []string{"fmt", "-w", savefile}...)
	if false {
		err = cmd.Run()
...
func (this *GenerateCy) saveCodeToFileWithCode(modname, file string, bcc string) {
	// qtx{yyy}, only yyy
	savefile := fmt.Sprintf("src/%s/%s.go", modname, file)
...
func (this *GenerateCy) genFileHeader(cursor, parent clang.Cursor) {
	this.genFileHeaderWithcp(this.cp, cursor, parent)
	this.genFileHeaderWithcp(this.cpnomin, cursor, parent)
}
func (this *GenerateCy) genFileHeaderWithcp(cp *CodePager, cursor, parent clang.Cursor) {
	file, line, col, _ := cursor.Location().FileLocation()
	if false {
		log.Printf("%s:%d:%d @%s\n", file.Name(), line, col, file.Time().String())
	}
...
func (this *GenerateCy) isinnomin(cursor clang.Cursor) bool {
	file, line, col, _ := cursor.Location().FileLocation()
	if false {
		log.Printf("%s:%d:%d @%s\n", file.Name(), line, col, file.Time().String())
	}
...
func (this *GenerateCy) walkClass(cursor, parent clang.Cursor) {
...
// 从xclass中查找是否有对应的方法
func (this *GenerateCy) filterClipqt() {
...
func (this *GenerateCy) protoMatch(c1, cx clang.Cursor) bool {
	c2 := cx
...
func (this *GenerateCy) genExterns(cursor, parent clang.Cursor) {
...
func (this *GenerateCy) genImports(cursor, parent clang.Cursor) {
	this.genImportsWithcp(this.cp, cursor, parent)
	this.genImportsWithcp(this.cpnomin, cursor, parent)
}
func (this *GenerateCy) genImportsWithcp(cp *CodePager, cursor, parent clang.Cursor) {
...
func (this *GenerateCy) genClassDef(cursor, parent clang.Cursor) {
	...
	this.cp.APf("body", "*/")
	// genTypeStruct
	this.cp.APf("body", "struct %s {", cursor.Spelling())
	if len(bcs) == 0 {
		this.cp.APf("body", "    // CObject &qtrt.CObject")
		this.cp.APf("body", "    qtrt.CObject")
		// this.cp.APf("body", "    cthis voidptr")
	} else {
		// this.cp.APf("body", "    cthis voidptr")
		for _, bc := range bcs {
	...
	if false {
		...
		this.cp.APf("body", "type %sITFx = %s",
			cursor.Spelling(), strings.Join(sumtypes, " | "))
		this.cp.APf("body", "func (this %sITFx) getCthis() voidptr {", cursor.Spelling())
		this.cp.APf("body", "  cthis := voidptr(0)")
		this.cp.APf("body", "  match this {")
		for _, bcname := range sumtypes {
			this.cp.APf("body", "    %s { cthis = this.getCthis() }", bcname)
		}
		this.cp.APf("body", "  }")
	...
	this.cp.APf("body", "    getCthis() voidptr")
	this.cp.APf("body", "    to%s() *%s", cursor.Spelling(), cursor.Spelling())
	this.cp.APf("body", "}")
	this.cp.APf("body", "func (ptr *%s) to%s() *%s { return ptr }",
...
func (this *GenerateCy) filter_base_classes(bcs []clang.Cursor) []clang.Cursor {
...
func (this *GenerateCy) genMethods(cursor, parent clang.Cursor) {
...
// 按名字/重载overload分组
func (this *GenerateCy) groupMethods() [][]clang.Cursor {
...
func ismthnomin_dep_cy(cursor clang.Cursor) bool {
	file, _, _, _ := cursor.Location().FileLocation()
...
func (this *GenerateCy) getpropercp(cursor clang.Cursor) *CodePager {
...
func (this *GenerateCy) genMethodFuncType(cursor, parent clang.Cursor, midx int) {
	...
	// log.Println(cursor.Spelling(), cursor.DisplayName(), cursor.Kind(), cursor.Language(), rety.Spelling(), rety.Kind())
	besret := MethodHasStructRet(cursor)
	if besret {
		// TODO x86, for x64 is more complex
		this.argDesc = append([]string{"sretobj voidptr"}, this.argDesc...)
		argStr = strings.Join(this.argDesc, ", ")
	}
	cp.APf("body", "    type Tfntype = func(%s) %s", argStr, retstr)
}
func (this *GenerateCy) genMethodHeader(cursor, parent clang.Cursor, midx int, isdv bool) {
...
func (this *GenerateCy) genMethodInit(cursor, parent clang.Cursor) {
	if cursor.Kind() == clang.Cursor_Constructor {
	}
	switch cursor.Kind() {
	case clang.Cursor_Constructor:
	...
	case clang.Cursor_Destructor:
		this.cp.APf("body", "func (this *%s) delete_%s(args...interface{}) {",
...
func (this *GenerateCy) genMethodSignature(cursor, parent clang.Cursor, midx int) {
	if cursor.Kind() == clang.Cursor_Constructor {
	}
	var cp = this.getpropercp(cursor)
...
func (this *GenerateCy) genMethodSignatureDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	if cursor.Kind() == clang.Cursor_Constructor {
	}
...
func (this *GenerateCy) genMethodSignatureNoThis(cursor, parent clang.Cursor, midx int) {
	this.genArgsDest(cursor, parent, true)
	argStr := strings.Join(this.destArgDesc, ", ")
	var cp = this.getpropercp(cursor)
...
func (this *GenerateCy) genMethodFooter(cursor, parent clang.Cursor) {
	var cp = this.getpropercp(cursor)
...
func (this *GenerateCy) genMethodFooterFFI(cursor, parent clang.Cursor, midx int) {
	var cp = this.getpropercp(cursor)
...
func (this *GenerateCy) genVTableTypes(cursor, parent clang.Cursor, midx int) {
...
func (this *GenerateCy) genNameLookup(cursor, parent clang.Cursor) {
	this.cp.AP("body", "")
...
func (this *GenerateCy) genCtor(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "    fnobj := Tfntype(0)")
	cp.APf("body", "    fnobj = qtrt.symQtfunc6(%s, \"%s\")",
		this.mangler.crc32(cursor), this.mangler.origin(cursor))
	cp.APf("body", "    cthis := qtrt.mallocraw(%d)", parent.Type().SizeOf())
	...
	cp.APf("body", "    qtrt.setFinalizer(vthis, delete%s)", parent.Spelling())
...
func (this *GenerateCy) genCtorDvs(cursor, parent clang.Cursor, midx int) {
	dvn := num_default_value(cursor)
	if dvn == 0 {
		return
	}
...
func (this *GenerateCy) genCtorDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	// log.Println(this.mangler.origin(cursor))
...
func (this *GenerateCy) genCtorFromPointer(cursor, parent clang.Cursor, midx int) {
	if midx > 0 { // 忽略更多重载
		return
	}
	...
	this.cp.APf("body", "func new%sFromptr(cthis voidptr) %s {",
...
func (this *GenerateCy) genYaCtorFromPointer(cursor, parent clang.Cursor, midx int) {
	if midx > 0 { // 忽略更多重载
		return
	}
...
func (this *GenerateCy) genGetCthis(cursor, parent clang.Cursor, midx int) {
	if midx > 0 { // 忽略更多重载
		return
	}
	bcs := find_base_classes(parent)
	...
	*/
	...
	if len(bcs) == 0 {
		...
		this.cp.APf("body", "    return this.CObject.getCthis()")
	} else {
		for _, bc := range bcs {
...
func (this *GenerateCy) genSetCthis(cursor, parent clang.Cursor, midx int) {
	if midx > 0 { // 忽略更多重载
		return
	}
	bcs := find_base_classes(parent)
...
func (this *GenerateCy) genDtor(cursor, parent clang.Cursor, midx int) {
	this.genMethodHeader(cursor, parent, midx, false)
	this.genMethodSignature(cursor, parent, midx)
	this.genMethodFuncType(cursor, parent, midx)
...
func (this *GenerateCy) genDtorNoCode(cursor, parent clang.Cursor, midx int) {
	// this.genMethodHeader(cursor, parent, midx)
	// this.genMethodSignature(cursor, parent, midx)
	clsname := cursor.Spelling()
	this.genDtorImpl(clsname, cursor)
}
func (this *GenerateCy) genDtorImpl(clsname string, cursor clang.Cursor) {
	// this.genMethodHeader(cursor, parent, midx)
	// this.genMethodSignature(cursor, parent, midx)
	var cp = this.getpropercp(cursor)
	...
	cp.APf("body", "func delete%s(this *%s) {", clsname, clsname)
	cp.APf("body", "    fnobj := qtrt.TCppDtor(0)")
	cp.APf("body", "    fnobj = qtrt.symQtfunc6(%d, \"%s\")", symcrc32(symbol), symbol)
	cp.APf("body", "    fnobj(this.getCthis())")
	cp.APf("body", "    that := this")
	cp.APf("body", "    //that.cthis = voidptr(0)")
	cp.APf("body", "}\n")

	cp.APf("body", "func (this *%s) freecpp() { delete%s(this) }\n",
		clsname, clsname)
	// for v compiler, do not need public
	cp.APf("body", "func (this *%s) free() {\n", clsname)
	cp.APf("body", "  /*delete%s(this)*/\n", clsname)
	cp.APf("body", "  cthis := this.getCthis()")
	cp.APf("body", "  //println(\"%s freeing ${cthis} %d bytes\")\n", clsname, 0)
	cp.APf("body", "}\n")
}

func (this *GenerateCy) genNonStaticMethod(cursor, parent clang.Cursor, midx int) {
	this.genParamsFFI(cursor, parent)
	this.paramDesc = append([]string{"this.getCthis()"}, this.paramDesc...)
	paramStr := strings.Join(this.paramDesc, ", ")
	_ = paramStr
	...
	if retype.Kind() != clang.Type_Void {
		if besret {
			cp.APf("body", "    sretobj := qtrt.mallocraw(%d)", retype.SizeOf())
...
func (this *GenerateCy) genNonStaticMethodDvs(cursor, parent clang.Cursor, midx int) {
	dvn := num_default_value(cursor)
	if dvn == 0 {
		return
	}
...
func (this *GenerateCy) genNonStaticMethodDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	this.genParamsFFI(cursor, parent)
	this.paramDesc = append([]string{"this.getCthis()"}, this.paramDesc...)
	paramStr := strings.Join(this.paramDesc, ", ")
	_ = paramStr
...
func (this *GenerateCy) genStaticMethod(cursor, parent clang.Cursor, midx int) {
	this.genParamsFFI(cursor, parent)
	paramStr := strings.Join(this.paramDesc, ", ")
...
func (this *GenerateCy) genStaticMethodDvs(cursor, parent clang.Cursor, midx int) {
	dvn := num_default_value(cursor)
	if dvn == 0 {
		return
	}
...
func (this *GenerateCy) genStaticMethodDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	this.genParamsFFI(cursor, parent)
	paramStr := strings.Join(this.paramDesc, ", ")
...
func (this *GenerateCy) genStaticMethodNoThis(cursor, parent clang.Cursor, midx int) {
...
func (this *GenerateCy) genNonVirtualMethod(cursor, parent clang.Cursor, midx int) {

}

func (this *GenerateCy) genProtectedCallbacks(cursor, parent clang.Cursor) {
	log.Println("process class:", len(this.methods), cursor.Spelling())
...
var inheritMethodsv_dep = map[string]int{}

func (this *GenerateCy) genProtectedCallback(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "func callback_%s(cthis voidptr %s) {", cursor.Mangling(), argStr)
	...
	if cursor.Kind() != clang.Cursor_Constructor && cursor.Kind() != clang.Cursor_Destructor {
		...
		if _, ok := inheritMethodsv[key]; !ok {
			...
			this.cp.APf("body", "func (this *%s) inherit_%s(f func(%s) %s) {",
...
func (this *GenerateCy) genArgs(cursor, parent clang.Cursor) {
...
func (this *GenerateCy) genArg(cursor, parent clang.Cursor, idx int) {
...
func (this *GenerateCy) genArgsDest(cursor, parent clang.Cursor, asitf bool) {
...
func (this *GenerateCy) genArgDest(cursor, parent clang.Cursor, idx int, asitf bool) {
...
func (this *GenerateCy) dvTrimArg(argsDesc []string, dvn int, dvidx int) []string {
	return argsDesc[:len(argsDesc)-dvn+dvidx]
}

// midx method index
func (this *GenerateCy) genArgsConv(cursor, parent clang.Cursor, midx int) {
	for idx := 0; idx < int(cursor.NumArguments()); idx++ {
		argc := cursor.Argument(uint32(idx))
...
// midx method index
// aidx method index
func (this *GenerateCy) genArgConv(cursor, parent clang.Cursor, midx, aidx int) {
...
// midx method index
func (this *GenerateCy) genArgsConvFFI(cursor, parent clang.Cursor, midx int) {
...
// midx method index
// aidx method index
func (this *GenerateCy) genArgConvFFI(cursor, parent clang.Cursor, midx, aidx int) {
	var cp = this.getpropercp(parent)

	argty := cursor.Type()
	barety := get_bare_type(argty)
	if TypeIsCharPtrPtr(argty) {
		cp.APf("body", "    convArg%d := qtrt.string_slice_to_ccharpp(%s)", aidx,
			this.genParamRefName(cursor, parent, aidx))
	} else if TypeIsCharPtr(argty) {
		cp.APf("body", "    convArg%d := qtrt.cstringr(&%s)", aidx,
			this.genParamRefName(cursor, parent, aidx))
		...
		cp.APf("body", "    tmpArg%d := %snewQString5(%s)", aidx, pkgPref,
			this.genParamRefName(cursor, parent, aidx))
		// this.cp.APf("body", "    defer %sDeleteQString(tmpArg%d)", pkgPref, aidx) // not needed
		cp.APf("body", "    convArg%d := tmpArg%d.getCthis()", aidx, aidx)
		// cp.APf("body", "    defer {tmpArg%d.freecpp()}", aidx)
		...
		if _, ok := privClasses[argty.PointeeType().Spelling()]; ok {
			...
			cp.APf("body", "    convArg%d := voidptr(0)", aidx)
...
// midx method index
func (this *GenerateCy) genArgsConvFFIDv(cursor, parent clang.Cursor, midx int, dvidx int) {
	log.Println("gggggggggg", cursor.Spelling(), cursor.ResultType().Kind(), cursor.ResultType().Spelling(), parent.Spelling())
	dvn := num_default_value(cursor)
...
// midx method index
// aidx method index
func (this *GenerateCy) genArgConvFFIDv(cursor, parent clang.Cursor, midx, aidx int, dvidx int) {
	...
	if TypeIsCharPtrPtr(argty) {
		...
		usemod := get_decl_mod(cursor)
		pkgPref := gopp.IfElseStr(usemod == "core", "", "qtcore.")
		cp.APf("body", "    convArg%d := %snew%s()", aidx, pkgPref, get_bare_type(argty).Spelling())
	} else if is_qt_class(argty) && get_bare_type(argty).Spelling() == "QChar" {
		usemod := get_decl_mod(cursor)
		pkgPref := gopp.IfElseStr(usemod == "core", "", "qtcore.")
		cp.APf("body", "    convArg%d  := %snewQChar8(`%s`)", aidx,
...
func (this *GenerateCy) genParams(cursor, parent clang.Cursor) {
...
func (this *GenerateCy) genParam(cursor, parent clang.Cursor, aidx int) {
...
func (this *GenerateCy) genParamRefName(cursor, _ clang.Cursor, aidx int) string {
	argName := cursor.Spelling()
	argName = gopp.IfElseStr(is_v_keyword(argName), argName+"_", argName)
...
func (this *GenerateCy) genParamsFFI(cursor, parent clang.Cursor) {
	this.paramDesc = make([]string, 0)
	for idx := 0; idx < int(cursor.NumArguments()); idx++ {
		argc := cursor.Argument(uint32(idx))
...
func (this *GenerateCy) genParamFFI(cursor, parent clang.Cursor, idx int) {
	argty := cursor.Type()
	if TypeIsCharPtrPtr(argty) {
...
func (this *GenerateCy) genRetFFI(cursor, parent clang.Cursor, midx int) {
	var cp = this.getpropercp(cursor)
...
func (this *GenerateCy) genArgsCGO(cursor, parent clang.Cursor) {
	this.argDesc = make([]string, 0)
	for idx := 0; idx < int(cursor.NumArguments()); idx++ {
		argc := cursor.Argument(uint32(idx))
...
func (this *GenerateCy) genArgCGO(cursor, parent clang.Cursor, idx int) {
	argty := cursor.Type()
...
func (this *GenerateCy) genArgsCGOSign(cursor, parent clang.Cursor) {
	this.argDesc = make([]string, 0)
	for idx := 0; idx < int(cursor.NumArguments()); idx++ {
		argc := cursor.Argument(uint32(idx))
...
func (this *GenerateCy) genArgCGOSign(cursor, parent clang.Cursor, idx int) {
	argty := cursor.Type()
	argName := gopp.IfElseStr(cursor.Spelling() == "", fmt.Sprintf("arg%d", idx), cursor.Spelling())
	argName = gopp.IfElseStr(is_v_keyword(argName), argName+"_", argName)
...
func (this *GenerateCy) genClassEnums(cursor, parent clang.Cursor) {
	...
	for _, enum := range this.enums {
		...
		if false { // disable namebyvalue
			...
			this.cp.APf("body", "}")
			this.cp.APf("body", "func %s%sItemName(val int) string {",
				cursor.DisplayName(), enum.DisplayName())
...
func (this *GenerateCy) genEnumsGlobal(cursor, parent clang.Cursor) {
	// log.Println("yyyyyyyy", cursor.DisplayName(), parent.DisplayName())
	...
	for _, enum := range this.enums {
		...
		if false { // disable enum name generate
			this.cp.APf("body", "func get%sItemName(val int) string {", enum.DisplayName())
			// this.cp.APf("body", "  match val {")
			enum.Visit(func(c1, p1 clang.Cursor) clang.ChildVisitResult {
				switch c1.Kind() {
				case clang.Cursor_EnumConstantDecl:
...
func (this *GenerateCy) genEnum() {

}

func (this *GenerateCy) genFunctions(cursor clang.Cursor, parent clang.Cursor) {
...
func (this *GenerateCy) genFunction(cursor clang.Cursor, olidx int) {
...
// only for static member
func (this *GenerateCy) genBareFunctionSignature(cursor, parent clang.Cursor, midx int) {
	this.genArgsDest(cursor, parent, true)
	argStr := strings.Join(this.destArgDesc, ", ")
...
func (this *GenerateCy) genConstantsGlobal(cursor, parent clang.Cursor) {
	dedups := map[string]int{}
