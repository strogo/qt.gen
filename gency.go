...
func (this *GenerateCy) genMethodFuncType(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "    // type Tfntype = func(%s) %s // TODO compiler", argStr, retstr)
	cp.APf("body", "    var cfnval func(%s) %s", argStr, retstr)
}
...
func (this *GenerateCy) genCtor(cursor, parent clang.Cursor, midx int) {
	...
	cp.APf("body", "    fnobj := cfnval // Tfntype(0)")
...
func (this *GenerateCy) genClassEnums(cursor, parent clang.Cursor) {
	...
	for _, enum := range this.enums {
		...
		this.cp.APf("body", "const /*%s%s*/ (", cursor.DisplayName(), enum.DisplayName())
		enum.Visit(func(c1, p1 clang.Cursor) clang.ChildVisitResult {
			switch c1.Kind() {
			case clang.Cursor_EnumConstantDecl:
...
func (this *GenerateCy) genEnumsGlobal(cursor, parent clang.Cursor) {
	...
	for _, enum := range this.enums {
		...
		this.cp.APUf("body", "const /*%s*/ (", enum.DisplayName())
		enum.Visit(func(c1, p1 clang.Cursor) clang.ChildVisitResult {
			switch c1.Kind() {
			case clang.Cursor_EnumConstantDecl:
				log.Println("yyyyyyyyy", c1.EnumConstantDeclValue(), c1.DisplayName(), p1.DisplayName(), cursor.DisplayName())
...
func (this *GenerateCy) genFunctions(cursor clang.Cursor, parent clang.Cursor) {
	...
	for _, qtmod := range qtmods {
		...
		writehead := func(cp *CodePager) {
			...
			cp.APf("header", "  // if false{_=unsafe.Pointer(0)}")
